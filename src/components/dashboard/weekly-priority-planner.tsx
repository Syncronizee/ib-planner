...
function buildInitialDraft(priorities: WeeklyPriority[]) {
  ...
  return Array.from({ length: WEEKLY_PRIORITY_SLOTS }, (_, index) => {
    ...
    return {
      ...
      scheduled_start_time: normalizePriorityTimeValue(priority?.scheduled_start_time) ?? null,
      scheduled_end_time: normalizePriorityTimeValue(priority?.scheduled_end_time) ?? null,
...
export function WeeklyPriorityPlanner({
  ...
  const savePriority = async (slot: number, draft: DraftPriority) => {
    ...
    const commonPayload = {
      ...
      scheduled_start_time: normalizePriorityTimeValue(draft.scheduled_start_time),
      scheduled_end_time: normalizePriorityTimeValue(draft.scheduled_end_time),
      is_completed: draft.is_completed,
